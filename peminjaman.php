 ...
 if (empty($_SESSION['login_user']) AND empty($_SESSION['login_pass'])){
  ...
  $query = mysql_query("select max(id_transaksipinjam) as maxID from transaksipinjam");
  $data = mysql_fetch_array($query);
  $idMax = $data['maxID'];
  $noUrut = (int) substr($idMax,3,7);
  $noUrut++;
  $newID = "TRP".sprintf("%07s",$noUrut);
 ?>
<!DOCTYPE html>
<html lang="en">
  <head>
    ...
    <title>Sirkulasi Perpustakaan - Peminjaman</title>
    ...
    <link href="assets/js/jquery.min.css" rel="stylesheet">
    <script>
        function table_load(){
            url="sirkulasi/peminjaman/data_display.php?table=pinjam"
            $("#table_area").load(url);
            }
        function insert_pinjam(){
            ...
            tgl_kembali=$("#tgl_kembali").val();
    				url="sirkulasi/peminjaman/data_insert.php?table=insert_pinjam&id_transaksipinjam="+id_transaksipinjam+"&no_induk="+no_induk+"&id_petugas="+id_petugas+"&id_buku="+id_buku+"&tgl_kembali="+tgl_kembali;
          ...
          function hapusPinjam(id_transaksipinjam){
      				var r = confirm("Anda yakin akan menghapus data?");
      				if (r == true){
    					url="sirkulasi/peminjaman/data_delete.php?table=delete_datapinjam&id_transaksipinjam="+id_transaksipinjam;
      ...
      ***********************************************************************************************************************************************************
      ...
      <section id="main-content">
          <section class="wrapper">
            	<h3><i class="fa fa-angle-right"></i> Sirkulasi / Peminjaman </h3>
                <div class="row mt">
                  <div class="col-lg-12">
                    <div class="form-panel tengah">
                        <button type="button" class="btn btn-round btn-success" data-toggle="modal" data-target="#myModalSimpan">Pinjam Buku</button>
                    </div>
                  </div>
                </div>
                <div class="row mt">
                    ...
                    <div class="form-panel">
                      <h4><i class="fa fa-angle-right"></i> Daftar Data Peminjaman </h4>
                      <section id="unseen">
      ...
      <!-- Modal Tambah Pinjam-->
          <div class="modal fade" id="myModalSimpan" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                <div class="modal-header">
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                  <h4 class="modal-title" id="myModalLabel">Transaksi Peminjaman Buku Perpustakaan</h4>
                </div>
                <div class="modal-body">
                  <form>
              			<div class="form-group">
                      <label>ID Transaksi Peminjaman</label>
              				<input type="text" class="form-control" id="id_transaksipinjam" placeholder="ID Transakipinjam" value="<?php echo $newID; ?>" readonly>
              			</div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                      ...
                      <select id="id_buku" class="selectpicker form-control" data-live-search="true">
                        ...
                        <?php
                            include 'connection.php';
                            $query="SELECT id_buku, judul_buku FROM buku where jumlah_buku > 0";
                            $result = mysql_query($query);
                    ...
                    <div class="form-group">
                      ...
                      <div class="bfh-datepicker" data-format="y-m-d" data-min="today" data-close="true" id="tgl_kembali"></div>
              			</div>
                  </form>
                </div>
                <div class="modal-footer">
                  <button type="button" class="btn btn-default" data-dismiss="modal">Tutup</button>
                  <button type="button" class="btn btn-primary" onclick="insert_pinjam();">Simpan Peminjaman</button>
                </div>
              </div>
            </div>
          </div>
